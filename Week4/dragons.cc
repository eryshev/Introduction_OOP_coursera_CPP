...
public:
    ...
    inline bool vivant() const { return points_de_vie_ > 0; }

    inline int points_attaque() const { return vivant() ? force_ * niveau_ : 0; }

    inline void deplacer(int d) { if(vivant()) position_ += d; }

    inline void adieux() const { cout << nom_ << " n'est plus!" << endl; }
    ...
    void afficher() const {
        cout << nom_ << ", niveau: " << niveau_ << ", points de vie: " << points_de_vie_ <<
                ", force: " << force_ << ", points d'attaque: " << points_attaque() << ", position: " << position_ << endl;
    }
};
...
class Dragon: public Creature {
    int portee_flamme_;
public:
    Dragon(string n, int niv, int pv, int f, int r, int pos = 0)
        : Creature(n, niv, pv, f, pos), portee_flamme_(r)
    {}

    inline void voler(int pos) { if(vivant()) position_ = pos; }
...
class Hydre: public Creature {
    ...
    int dose_poison_;
public:
    Hydre(string n, int niv, int pv, int f, int l_c, int d_p, int pos = 0)
        : Creature(n, niv, pv, f, pos), longueur_cou_(l_c), dose_poison_(d_p)
    {}
